...
def handle_gaps(finestra_aggiunta_gap_query: str, finestra_aggiunta_gap_sub: str, finestra_mismatch_query: str,
                finestra_mismatch_sub: str, gap_size: int, gap_target: str, x_max: int) -> str:
    """
    ...
    sequence_sub_list = []
    score_list = []
    gap_numbers = min(gap_size,max_gap) #gap size uguale alla differenza
    # viene calcolato in modo da capire quanti gap inserire, per limitarne l'espansione

    if gap_target == 'query':
        for a in range(gap_numbers):
            sequence_finestra_gap = '_' * (
                        a + 1) + finestra_mismatch_query  # si inserisce un numero di gap all'inizio della finestra in cui abbiamo mismatch
...
################## MELANIA #################
def extend_seed_right(sequence_query_ext: str, sequence_sub_ext: str, gap_size: int, perform_gap: bool, gap_target: str,
                      x_max: int):
    """
    ...
    if perform_gap:
        ...
        while gap_used < gap_size:
            ...
            if mismatch_consecutivi == x_max:
                ...
                sequence_query, sequence_subject = handle_gaps(finestra_aggiunta_gap_query, finestra_aggiunta_gap_sub,
                                                               finestra_mismatch_query, finestra_mismatch_subject,
                                                               gap_rimasti, gap_target, x_max)
                ...
                if i < len(seeds) - 1:
                    ...
                    if next_seed:
                        ...
                        try:
                            ...
                            print(
                                f"Errore: I valori di next_start_q, next_start_s, next_end_q, next_end_s devono essere interi. Seed: {next_seed}")
                            continue
...
def create_results_table(cont_hsp_query: list, cont_hsp_sub: list, cont_hsp_score: list, col: list,
...
def blast_result_df(best_alignment_query: pd.DataFrame, query_partenza: tuple, list_partenza_subject: list,
                    filename: str) -> pd.DataFrame:
    """
    ...
    coverage_cont, e_value_cont, identity_cont = metrics_for_blast(best_alignment_query, query_partenza,
                                                                   list_partenza_subject)
